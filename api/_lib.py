...
def _gemini_prompt(camera: dict, scenario: dict, has_video: bool = False) -> str:
    video_note = (
        "An 8-second CCTV clip generated by Veo 3.1 is attached as the first "
        "input. Watch it frame-by-frame and base every finding on what you "
        "actually see in the video. The narrative below is the upstream "
        "scenario prompt — use it only as a hint; trust the video over the text."
        if has_video else
...
scenario narrative (upstream hint, do NOT prefer over the video):
{scenario['narrative']}
...
def call_gemini(
    ...
    If video_bytes is provided, attaches the MP4 inline so Gemini analyzes the
    actual frames rather than narrating from text alone. Falls back to text-
    only if no bytes are supplied.
    """
    prompt = _gemini_prompt(camera, scenario, has_video=bool(video_bytes))
...
def fetch_clip_bytes(scenario_id: str, host: Optional[str]) -> Optional[bytes]:
    """Fetch the pre-baked Veo MP4 for a scenario.

    Tries bundled local file first (in case includeFiles is wired), then falls
    back to a same-deployment HTTP fetch using the host from the request.
    """
    if not scenario_id:
        return None
    # Local first
    local = ROOT / "clips" / f"{scenario_id}.mp4"
    if local.exists():
        try:
    ...
    if not host:
        return None
    url = f"https://{host}/clips/{scenario_id}.mp4"
    try:
